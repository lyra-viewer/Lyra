...
internal sealed class PresentedPeak
{
    ...
    public void Observe(SKSurface surface, string backend, bool hasContent)
    {
        ...
        {
            Logger.Debug($"[PresentedPeak] [{backend}] Could not sample the presented peak: {ex.Message}");
        }
    }
    ...
    private static void Report(SKSurface surface, string backend)
    {
        ...
        {
            Logger.Debug($"[PresentedPeak] [{backend}] Presented peak unavailable: the surface would not read back.");
            return;
        }

        var peak = Peak(pixels);

        Logger.Info($"[PresentedPeak] [{backend}] Peak value presented: {peak:0.###}x SDR white " +
                    (peak > 1.001f ? "- the frame is carrying extended range." : "- nothing above SDR white was drawn."));
    }
