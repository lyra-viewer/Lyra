...
public abstract class SkiaRendererBase : IDisposable, IDrawableSizeAware
{
    ...
    protected SkiaRendererBase(IDropStatusProvider dropStatusProvider)
    {
    ...
    {
        var bounds = new DrawableBounds(WindowWidth, WindowHeight);
        ...
        {
            _centeredOverlay.Render(canvas, bounds, textColor,
                $"{drop.FilesSupported} images found, {drop.FilesEnumerated} files scanned...");
        }
        else if (_composite is null || _composite.IsEmpty)
        {
            _centeredOverlay.Render(canvas, bounds, textColor, _composite?.State == CompositeState.Loading ? "Loading..." : "No image");
        }
    }
